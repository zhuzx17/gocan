...
| `WithSLCANSerialBaud(int)` | `115200` | SLCAN 主机串口速率 | ✓ | ✓ |
| `WithSLCANSilent(bool)` | `false` | CANable 监听模式 (`M0/M1`) | ✓ | ✓ |
| `WithSLCANAutoRetransmit(bool)` | `true` | CANable 自动重传 (`A0/A1`) | ✓ | ✓ |
...
以下选项只由 `OpenSLCAN` / `OpenSLCANFD` 使用；传给 PCAN 或 SocketCAN 入口时忽略：

```go
gocan.WithSLCANSerialBaud(115200)
gocan.WithSLCANSilent(true)
gocan.WithSLCANAutoRetransmit(false)
```

CANable 2.0 是 USB CDC 设备，通常忽略主机设置的串口 baud；保留该选项是为了兼容
使用 UART/USB 转串口的同协议设备。模式和自动重传命令在通道打开前发送。
