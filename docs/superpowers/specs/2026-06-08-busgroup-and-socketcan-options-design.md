...
当前 `Frame` 类型没时间戳字段。本设计**不改 Frame**，避免污染热路径。新增方法：

```go
// ReadOneTimestamped 与 ReadOne 相同，但额外返回内核时间戳。
// 当 Open 时未启用 WithRecvTimestamp 时，返回零值 time.Time（非错误，文档化语义）。
func (b *Bus) ReadOneTimestamped(ctx context.Context) (Frame, time.Time, error)
```

`Bus.Receive() <-chan Frame` 与 `BusGroup.Receive() <-chan SourcedFrame` 保持只发 Frame，不带时间戳——需要时间戳的高级用法显式走 `ReadOneTimestamped`。

**BusGroup + 时间戳**：BusGroup 路径下需要时间戳的业务，从 `group.Get(name)` 拿到 `*Bus` 后直接调 `bus.ReadOneTimestamped`，跳过 `group.Receive()`。这样设计的理由是：启用时间戳的场景往往对延迟和精度敏感，多一层 fan-in goroutine 反而会引入不必要的 channel 延迟和拷贝。文档里会明确这一点。
...
- Frame 加 RxTime 字段（避免污染热路径，时间戳走单独方法）
