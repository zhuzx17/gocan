...
```
can: CAN_Read failed: 0x00000040: bus light error
```
...
err := bus.Send(ctx, fr)
switch {
case errors.Is(err, gocan.ErrBusOff):
    log.Println("总线断开，尝试 Reset 恢复")
    _ = bus.Reset()
case errors.Is(err, gocan.ErrQueueXmtFull):
...
for {
    select {
    case e := <-bus.Errors():
        if errors.Is(e, gocan.ErrBusOff) {
